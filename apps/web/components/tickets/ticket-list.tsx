      ...
      <CardHeader className="flex flex-row items-center justify-between">
        ...
        {!adminView && (
          <Link href={`/${locale}/g/tickets/new`}>
            <Button>
              <Plus className="mr-2 h-4 w-4" /> {tTickets.list?.create || 'New Ticket'}
            </Button>
          </Link>
        )}
      </CardHeader>
          ...
          <div className="space-y-4">
            {tickets.map((ticket) => (
              <Link
                href={
                  adminView
                    ? `/${locale}/s/tickets/${ticket.id}`
                    : `/${locale}/g/tickets/${ticket.id}`
                }
                key={ticket.id}
                className="block border rounded p-4 hover:bg-muted/50 transition"
              >
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      ...
                      {ticket.createdBy &&
                        ` • ${tTickets.list?.by || 'By'} ${ticket.createdBy.name || ticket.createdBy.email}`}
                    </p>
                  </div>
                  <span
