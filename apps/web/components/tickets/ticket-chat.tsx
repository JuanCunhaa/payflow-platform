...
export function TicketChat({ ticketId }: { ticketId: string }) {
  ...
  const closeTicket = async () => {
    if (!confirm(tTickets.chat?.confirmClose || 'Are you sure you want to close this ticket?'))
      return;
      ...
      <div className="p-4 border-b flex justify-between items-center bg-card">
        <div>
          ...
          <div className="flex gap-2 items-center mt-1">
            ...
            {ticket.escalatedToPid && (
              <span className="text-xs bg-red-100 text-red-800 px-1 rounded">
                {tTickets.chat?.escalatedLabel || 'Escalated'}
              </span>
            )}
          </div>
        </div>
        ...
        {ticket.messages.map((msg: any) => {
          ...
          return (
            <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
              <div
                className={`max-w-[80%] rounded p-3 ${isMe ? 'bg-primary text-primary-foreground' : 'bg-muted'}`}
              >
                <p className="text-sm shadow-sm">{msg.message}</p>
                <span
                  className={`text-[10px] opacity-70 block text-right mt-1 ${isMe ? 'text-primary-foreground' : ''}`}
                >
