...
test.describe('School classes UI', () => {
  test('can create grade and class and see them listed', async ({ page }) => {
    ...
    await expect(page.getByText('Séries e turmas', { exact: false })).toBeVisible();
    ...
    await page.getByLabel('Nome da série').fill(gradeName);
    ...
    await page.getByRole('combobox').last().selectOption({ label: gradeName });

    await page.getByRole('button', { name: 'Salvar' }).click();

    await expect(page.getByText(className)).toBeVisible();
    await expect(page.getByRole('listitem').filter({ hasText: gradeName }).first()).toBeVisible();
  });
});
