...
test.describe('School settings and audit', () => {
  test('school admin can update settings and audit shows change', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Configurações da escola' })).toBeVisible();
    ...
    await expect(page.getByText('Configurações da escola salvas com sucesso.')).toBeVisible();
    ...
    await expect(page.getByText('Detalhes do evento', { exact: false })).toBeVisible();

    await expect(page.getByText(newDisplayName, { exact: false })).toBeVisible();
  });
});
