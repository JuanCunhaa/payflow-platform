...
test.describe('Password reset web flow', () => {
  test('forgot password validates email and shows generic success', async ({ page }) => {
    await page.goto(`/${LOCALE}/forgot-password`);

    await page.getByRole('button', { name: 'Enviar instruções' }).click();

    await expect(page.getByText('Informe um email válido.', { exact: false })).toBeVisible();
  ...
  test('reset password shows error for invalid token', async ({ page }) => {
    ...
    await page.getByLabel('Nova senha', { exact: true }).first().fill('NovaSenha123');
    await page.getByLabel('Confirmar nova senha', { exact: true }).first().fill('NovaSenha123');

    await page.getByRole('button', { name: 'Redefinir senha' }).click();

    await expect(page.getByText('Link de redefinição inválido', { exact: false })).toBeVisible();
  });
});
